...
GLfloat lightOnePosition[] = {400, 100, 1500, 1};
...
GLfloat lightZeroPosition[] = {900, 200, 300, 1};
GLfloat lightZeroColor[] = {.3, .3, .3, .3};
int lightflag=1,randomColorFlag=0;
...
void processMenuExit(int option) //function for menu exit
{
	if(option==0)
		exit(0);
}

void processMenuBgColor(int option) //function for menu background
{
	switch(option)
	{
...
void processMenuLighting(int option) //function for light off or on
{
	switch(option)
	{
		case 0:
...
void processMenuMain2(int option) // function for 2nd page menu
{
}
...
void processfullscreen(int option)
{
	...
	case 0:
		...
		break;
	case 1:
		glutReshapeWindow(1350,690);
		glutPositionWindow(0,0);
	}
}

void createGLUTMenus2() //function for menu
{
	int menuSolveCompletely = glutCreateMenu(processMenuSolveCompletely);
	glutAddMenuEntry("Press S to Start",0);
	...
	int menuscreen = glutCreateMenu(processfullscreen);
	...
	glutAddSubMenu("Full Screen",menuscreen);
...
void strokeString(float x,float y,float sx,float sy,char *string,int width)
{
	...
	for(c=string; *c != '\0'; c++)
	{
		glutStrokeCharacter(GLUT_STROKE_ROMAN, *c); //renders a stroke character using OpenGL
	}
	glPopMatrix();
}
...
void render(void)
{
	...
	glClear(GL_COLOR_BUFFER_BIT);
		glColor3f(0,1,0);
		strokeString(415,100,0.3,-0.3,"CITY ENGINEERING COLLEGE",4);
		strokeString(80,165,0.3,-0.3,"DEPARTMENT OF COMPUTER SCIENCE",4);
		strokeString(890,165,0.3,-0.3,"AND ENGINEERING",4);
		strokeString(220,230,0.3,-0.3,"COMPUTER GRAPHICS MINI PROJECT ON",2);
		glColor3f(0,1,1);
		strokeString(425,295,0.4,-0.4,"TOWER OF HANOI",6);
		strokeString(150,450,0.2,-0.2,"NUMBER OF DISKS:",3);
		glColor3f(1,1,0);
		char str[5];
		sprintf(str, "%d", NUM_DISCS);
		strokeString(450,450,0.2,-0.2,str,3);
		glColor3f(1,0,1);
		strokeString(50,500,0.17,-0.17,"1 . Set the number of disks using the Code at NUM_DISCS",2);
		strokeString(50,550,0.17,-0.17,"2 . Press (N) to start the simulation",2);
		strokeString(890,500,0.15,-0.15,"By:",2);
		glColor3f(1,0,0);
		strokeString(890,550,0.18,-0.18,"Parveen Taj",3);
		glColor3f(1,0,0);
		strokeString(890,600,0.18,-0.18,"Anjana Raghavendra",3);
		glutSwapBuffers();
